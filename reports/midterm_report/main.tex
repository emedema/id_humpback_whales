...
Over the years, a lot of research has been done in this field. There have been multiple Kaggle competitions and there are a multitude of approaches to this problem. While there have been some studies on the application of transfer learning augmented deep learning and classical machine learning \cite{YuanHongchun2020AAIC}, it begs the comparison of the best performing classical machine learning model (according to the kaggle competition) as well as the deep learning model itself (without transfer learning). 
...
\begin{itemize}
    \item Preprocessing the data to remove noise via [insert here]
...
The competition contains thousands of images of humpback whale flukes. Individual whales have been identified by researchers and given an identification (Id). The challenge is to predict the whale Id of images in the test set. What makes this such a challenge is that there are only a few examples for over 3,000 whale Ids. To compensate for this we will be utilizing data augmentation as discussed later in this section. First, we must clean the dataset of noise, such as the background in the images and normalize the images so that there is as little variation as possible within the dataset except for the differences between individual whale's flukes.

%TODO: refine this.
We will crop, scale, and resize the images such that all of the images are the same size and are only of the tail fin. This will ensure that all of our data is standardize and that we have less variance. We will do this by [Stephen please add what you did]
